...
subroutine test_degenerate (tests)
    ...
    call tc%assert_true (status_contains (status, NF_STATUS_INVALID_ARG), &
    ...
    call tc%assert_true (m == x1(1) .and. (status_contains (status, NF_STATUS_OK)), &
        "mean: 1d input, size(x) = 1")

    m = -1
    call std (x1, s, m, status=status)
    call tc%assert_true (m == x1(1) .and. s == 0.0_PREC .and. (status_contains (status, NF_STATUS_OK)), &
        "std: 1d input, size(x) = 1")


    x21(:, 1) = [7.234, -0.223423]
    call mean (x21, m1, dim=2, status=status)
    call tc%assert_true (all(x21(:, 1) == m1) .and. (status_contains (status, NF_STATUS_OK)), &
        "mean: 2d input of shape [2, 1]")

    x12(1, :) = [789.213, -2.123]
    call mean (x12, m1, dim=1, status=status)
    call tc%assert_true (all(x12(1, :) == m1) .and. (status_contains (status, NF_STATUS_OK)), &
        "mean: 2d input of shape [1, 2]")

    x21(:, 1) = [1.2345, -0.3214]
    s1 = -1.0
    call std(x21, s1, m1, dim=2, status=status)
    call tc%assert_true (all(x21(:, 1) == m1) .and. all(s1 == 0.0_PREC) .and. (status_contains (status, NF_STATUS_OK)), &
        "std: 2d input of shape [2, 1]")

    s1 = -1.0
    x12(1, :) = [0.213, -234.123]
    call std(x12, s1, m1, dim=1, status=status)
    call tc%assert_true (all(x12(1, :) == m1) .and. all(s1 == 0.0_PREC) .and. (status_contains (status, NF_STATUS_OK)), &
...
subroutine test_1d (tests)
    ...
    x = [real (PREC) :: (i, i = 1, N)]
    call mean (x, m, status=status)
    call tc%assert_true (abs(m - (N+1)/2) < 1d-15 .and. (status_contains (status, NF_STATUS_OK)), &
        "Mean of 1d sequence")
    ...
    call tc%assert_true (m == m2 .and. abs(s-s2) < 1d-15 .and. (status_contains (status, NF_STATUS_OK)), &
        "Std of 1d sequence")
...
subroutine test_2d (tests)
    ...
    call tc%assert_true (all(abs(m-m2) < tol) .and. (status_contains (status, NF_STATUS_OK)), &
        "Mean of 2d array along dim=1")

    call std (x, s, m, dim=1, status=status)
    call tc%assert_true (all(abs(m-m2) < tol) .and. all(abs(s-s2) < tol) .and. (status_contains (status, NF_STATUS_OK)), &
