...
c
      real (PREC) a,absc,abserr,b,centr,dhlgth,
     *  d1mach,epmach,fc,fsum,fval1,fval2,fv1,fv2,hlgth,resabs,resasc,
     *  resg,resk,reskh,result,uflow
      integer j,jtw,jtwm1
      procedure (f_integrand) :: f
c       Declare these as double precision regardless of API precision used,
c       to avoid warnings due to DATA statements.
...
c
   ...
   20 continue
      ...
      if(resasc.ne.0.0_PREC.and.abserr.ne.0.0_PREC)
     *  abserr = resasc*min(0.1e+01_PREC,
     *  (0.2e+03_PREC*abserr/resasc)**1.5_PREC)
      if(resabs.gt.uflow/(0.5e+02_PREC*epmach)) abserr = max
     *  ((epmach*0.5e+02_PREC)*resabs,abserr)
      return
      end
